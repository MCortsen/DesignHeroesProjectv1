...
namespace yeetles_13
{
    public class Database
    {
        ...
        public static List<string> GetAssignmentsByProject(int projectId, string userName)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(ConnectionString))
            {
                try
                {
                    MySqlCommand command = new MySqlCommand($"SELECT Title FROM ProjectAssignment_Table WHERE fk_UserID = {GetUserIdByUserName(userName)} AND fk_ProjectID = {projectId}", conn);
                    command.Connection.Open();
                    MySqlDataReader reader = command.ExecuteReader();
                    while (reader.Read())
                    {
                        assignments.Add(reader.GetString(0));
                    }
                    command.Connection.Close();
                    return assignments;
                }
